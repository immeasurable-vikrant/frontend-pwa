...
const ReviewComment = ({
			...
			if (rating && content) {
				...
				if (course_no) {
					...
					return sleep(300).then(() => {
						fetchAddComment(course_no, page, limit, content, rating, helpful, failed);
					});
				}
			}
		} else {
	...
	const renderTextField = ({ input, label, type, placeholder, multiLine, meta: { touched, error }, ...custom }) => (
			...
			<div>
				<div style={{ textAlign: 'center', marginTop: 30 }}>
					<Rating
						...
						readonly={false}
						empty={<img src={`/public/assets/images/star-grey.png`} alt='star' className='icon' />}
						placeholder={<img src={`/public/assets/images/star-yellow.png`} alt='star' className='icon' />}
						full={<img src={`/public/assets/images/star-yellow.png`} className='icon' alt='icon' />}
					...
					<div className='row'>
						<div className='col-sm-12'>
							<Paper
								...
								<br />
								<div className='text-size-second text-bold' style={{ textAlign: 'center' }}>
...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		fetchAddComment: (course_no, page, limit, content, rating, helpful, failed) =>
			dispatch(addComment(course_no, page, limit, content, rating, helpful, failed)),
		fetchUserInfo: () => dispatch(userInfo()),
	};
};

export default withRouter(form(connect(mapStateToProps, mapDispatchToProps)(ReviewComment)));
